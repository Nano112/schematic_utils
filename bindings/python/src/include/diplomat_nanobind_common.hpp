...
namespace nanobind::detail
{
    ...
    struct type_caster<nucleation::diplomat::result<T, E>>
    {
        ...
        using Value = nucleation::diplomat::result<T, E>;
        ...
        operator Value() {
            if (is_ok) {
                ...
                return nucleation::diplomat::Err<E>(forward_like_<V>(err_val.value()));
            }
        }
    ...
    class type_caster<nucleation::diplomat::span<T, E>> {
    ...
    public:
        using Value = nucleation::diplomat::span<T, E>;
        Value value = nucleation::diplomat::span<T, E>();
        ...
        bool from_python(handle src, uint8_t flags, cleanup_list* cleanup) noexcept {
            ...
            if constexpr (is_ndarray_scalar_v<T>) {
                ...
                if (caster.from_python(src, local_flags, cleanup)) {
                    ...
                    value = nucleation::diplomat::span<T, E>(caster.value.data(), caster.value.shape(0));
                    return true;
                }
            }
            ...
            if (nb::inst_check(src) && nb::isinstance<U>(src)) {
                ...
                value = nucleation::diplomat::span<T, E>(reinterpret_cast<T*>(bound_vec->data()), bound_vec->size());
                return true;
            }
            ...
            if (std::is_const_v<T> &&
                ...
                ListCaster caster;
                if (caster.from_python(src, local_flags, cleanup)) {
                    value = nucleation::diplomat::span<T, E>(reinterpret_cast<T*>(caster.value.data()), caster.value.size());
        ...
        static handle from_cpp(nucleation::diplomat::span<T, E> src, rv_policy policy, cleanup_list* cleanup) {
    ...
    template <>
    struct type_caster<nucleation::diplomat::string_view_for_slice> {
        NB_TYPE_CASTER(nucleation::diplomat::string_view_for_slice, const_name("str"))
        ...
        bool from_python(handle src, uint8_t, cleanup_list *) noexcept {
            ...
            value = nucleation::diplomat::string_view_for_slice(str, (size_t) size);
            return true;
        }

        static handle from_cpp(nucleation::diplomat::string_view_for_slice value, rv_policy, cleanup_list *) noexcept {
...
struct get_diplomat_result<nucleation::diplomat::result<T, E>> : std::true_type {
